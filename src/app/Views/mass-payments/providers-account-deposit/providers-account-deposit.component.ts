...
@Component({
  selector: 'app-providers-account-deposit',
  standalone: false,
  templateUrl: './providers-account-deposit.component.html',
  styleUrls: ['./providers-account-deposit.component.css'],
  providers: [ProvidersAccountDepositService, TicketsService, UtilsService, UifService]
})

export class ProvidersAccountDepositComponent implements OnInit {
  ...
  globalGloss!: string;
  warningMessagesTitle = 'Pago a proveedores abono en cuenta';
  successfulTransactionMessage = Constants.successfulTransactionMessage;
  accountDto: AccountDto;
  spreadsheetPerPage!: ProvidersPaymentsSpreadsheetsResult[];
  batchInformation: ProvidersPaymentData = new ProvidersPaymentData();
  ...
  constructor(private providersAccountDepositService: ProvidersAccountDepositService, private globalService: GlobalService,
    private ticketsService: TicketsService, private utilsService: UtilsService, private cdRef: ChangeDetectorRef, private userService: UserService,
    ...
    this.batchInformation.operationTypeId = OperationType.pagoProveedores;
    this.isAuthorizeOperation = this.userService.getUserToken().authorize_operation!;
    localStorage.setItem('operationType', OperationType.pagoProveedores.toString());
  }
  ...
  getPreviousSpreadsheets() {
    this.providersAccountDepositService.getPreviousSpreadsheets()
      .subscribe({next: response => this.previousSpreadsheets = response, error: _err => this.haveOldPayments = false});
  }

  getFavoriteTransactions() {
    this.providersAccountDepositService.getFavorites().subscribe({next: response => {
  ...
  handleSelectSpreadsheetId($event: MassivePaymentsPreviousFormResult) {
    this.providersAccountDepositService.getSpreadsheet(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
  ...
  handleSelectFavorite($event: MassPaymentFavoriteTransactions) {
    switch ($event.operation) {
      case Operation.load:
        this.providersAccountDepositService.getFavoriteDetail(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
      ...
      case Operation.update:
        this.providersAccountDepositService.updateFavorite(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
          .subscribe({next: _response => this.globalService.warning('Operación realizada: ', ' ', true), error: _err => this.globalService.warning(this.warningMessagesTitle, _err.message)});
        break;
    }
  }

  handleRemoveFavorite() {
    this.providersAccountDepositService.deleteFavorite(new MassivePaymentsSpreadsheetsDto({ id: this.selectedFavoriteTransaction }))
      .subscribe({next: response => {
  ...
  handleLoadFile($event: FormData) {
    this.providersAccountDepositService.getSpreadsheetFromFile($event)
      .subscribe({next: spreadsheet => this.updateSpreadsheet(spreadsheet),
      error: _err => this.globalService.warning(this.warningMessagesTitle, _err.message)});
  }

  updateSpreadsheet(spreadsheet: ProvidersPaymentsSpreadsheetsResult[]) {
    this.batchInformation.spreadsheet = Object.assign([], spreadsheet);
  ...
  handleTokenSubmit($event: TokenCredentials) {
    ...
    this.providersAccountDepositService.save(this.batchInformation)
      .subscribe({next: response => {

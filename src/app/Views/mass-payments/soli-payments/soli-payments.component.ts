...
@Component({
  selector: 'app-soli-payments',
  standalone: false,
  templateUrl: './soli-payments.component.html',
  styleUrls: ['./soli-payments.component.css'],
  providers: [SoliPaymentsService, ExchangeRatesService, UtilsService, TicketsService, UifService]
})
export class SoliPaymentsComponent implements OnInit {
  ...
  warningMessagesTitle = 'Pago Soli';
  successfulTransactionMessage = Constants.successfulTransactionMessage;
  accountDto: AccountDto;
  spreadsheetPerPage!: SoliPaymentsSpreadsheetsResult[];
  batchInformation: SoliPaymentData = new SoliPaymentData();
  ...
  constructor(private soliService: SoliPaymentsService, private globalService: GlobalService, private ticketsService: TicketsService,
    ...
    this.batchInformation.operationTypeId = OperationType.pagoSoli;
    this.isAuthorizeOperation = this.userService.getUserToken().authorize_operation!;
    localStorage.setItem('operationType', OperationType.pagoSoli.toString());
  }
  ...
  handleSelectSpreadsheetId($event: MassivePaymentsPreviousFormResult) {
    this.soliService.getSpreadsheet(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
  ...
  handleSelectFavorite($event: MassPaymentFavoriteTransactions) {
    switch ($event.operation) {
      case Operation.load:
        this.soliService.getFavoriteDetail(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
      ...
      case Operation.update:
        this.soliService.updateFavorite(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
  ...
  handleRemoveFavorite() {
    this.soliService.deleteFavorite(new MassivePaymentsSpreadsheetsDto({ id: this.selectedFavoriteTransaction }))
      .subscribe({next: response => {
  ...
  handleLoadFile($event: FormData) {
    this.soliService.getSpreadsheetFromFile($event)
      .subscribe({next: spreadsheet => this.updateSpreadsheet(spreadsheet),
      error: _err => this.globalService.warning(this.warningMessagesTitle, _err.message)});
  }

  updateSpreadsheet(spreadsheet: SoliPaymentsSpreadsheetsResult[]) {
    ...
    this.uifService.validateOriginDestinationUif(this.batchInformation)
      .subscribe({next: (_res) => {
        for (let i = 0; i < 1; i++) {
          ...
          this.batchInformation.uif[i].causalTransaction = 'RECAU';
  ...
  handleTokenSubmit($event: TokenCredentials) {
    ...
    this.soliService.save(this.batchInformation)
      .subscribe({next: response => {

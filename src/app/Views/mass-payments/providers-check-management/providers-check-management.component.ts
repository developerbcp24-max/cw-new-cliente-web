...
export class ProvidersCheckManagementComponent implements OnInit {
  ...
  constructor(private checkManagementService: ProvidersCheckManagementService, private globalService: GlobalService, private ticketsService: TicketsService,
    ...
    this.batchInformation.operationTypeId = OperationType.pagoProveedoresChequeGerencia;
    this.isAuthorizeOperation = this.userService.getUserToken().authorize_operation!;
    localStorage.setItem('operationType', OperationType.pagoProveedoresChequeGerencia.toString());
  }
  ...
  getPreviousSpreadsheets() {
    this._typeOperatio.OperationType=26;
  ...
  getFavoriteTransactions() {
    this._opertaionType.OperationTypeId=26;

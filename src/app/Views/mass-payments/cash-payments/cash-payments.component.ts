...
import { CashPaymentsService } from 'src/app/Services/mass-payments/cash-payments.service';
...
@Component({
  ...
  providers: [CashPaymentsService, ExchangeRatesService, UtilsService, TicketsService, UifService]
})
export class CashPaymentsComponent implements OnInit {
  ...
  spreadsheetPerPage!: CashPaymentsSpreadsheetsResult[];
  batchInformation: CashPaymentData = new CashPaymentData();
  ...
  constructor(private cashService: CashPaymentsService, private globalService: GlobalService, private ticketsService: TicketsService,
    ...
    this.batchInformation.operationTypeId = OperationType.pagoProveedoresEfe;
    this.isAuthorizeOperation = this.userService.getUserToken().authorize_operation!;
    localStorage.setItem('operationType', OperationType.pagoProveedoresEfe.toString());
  }
  ...
  handleSelectSpreadsheetId($event: MassivePaymentsPreviousFormResult) {
    this.cashService.getSpreadsheet(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
  ...
  handleSelectFavorite($event: MassPaymentFavoriteTransactions) {
    switch ($event.operation) {
      case Operation.load:
        this.cashService.getFavoriteDetail(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
      ...
      case Operation.update:
        this.cashService.updateFavorite(new MassivePaymentsSpreadsheetsDto({ id: $event.id }))
  ...
  handleRemoveFavorite() {
    this.cashService.deleteFavorite(new MassivePaymentsSpreadsheetsDto({ id: this.selectedFavoriteTransaction }))
      .subscribe({next: response => {
  ...
  handleLoadFile($event: FormData) {
    this.cashService.getSpreadsheetFromFile($event)
      .subscribe({next: spreadsheet => this.updateSpreadsheet(spreadsheet),
      error: _err => this.globalService.warning(this.warningMessagesTitle, _err.message)});
  }

  updateSpreadsheet(spreadsheet: CashPaymentsSpreadsheetsResult[]) {
  ...
  handleTokenSubmit($event: TokenCredentials) {
    ...
    this.cashService.save(this.batchInformation)
      .subscribe({next: response => {

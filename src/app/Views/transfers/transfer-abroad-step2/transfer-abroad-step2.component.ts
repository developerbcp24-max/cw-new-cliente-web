...
@Component({
  selector: 'app-transfer-abroad-step2',
  standalone: false,
  templateUrl: './transfer-abroad-step2.component.html',
  styleUrls: ['./transfer-abroad-step2.component.css'],
  providers: [TransfersAbroadService, AccountsService]
})
export class TransferAbroadStep2Component implements OnInit, OnChanges {
  ...
  @Output() onSaveFavorite = new EventEmitter();
  @Output() onSaveTransfer = new EventEmitter<TransferAbroadDetailDto>();
  @Output() onSaveUif = new EventEmitter<TransferAbroadDto>();
  ...
  handlePayerBank($event: DestinationBankResult) {
    ...
    if (this.payerBank.code === '') {
      this.globalService.info('', 'No existe el banco seleccionado')
    }
  }
  ...
  validateForms(isValidRequester: boolean
    , isValidVeneficiary: boolean
    , isValidCategoryAsfi: boolean
    , isValidPayerBank: boolean
    , isValidIntermediaryBank: boolean) {
  ...
  handleSubmitDetail() {
    ...
    transferAbroadDetailDto.fundSource = this.resultFunds.fundSource;
    transferAbroadDetailDto.fundDestination = this.resultFunds.fundDestination;
    ...
    this.transfersAbroadService
      ...
      .subscribe({
        next: (res: ProcessBatchResult) => {
          if (this.isFrecuentTransferSave) {
            this.globalService.success('Servicio de transferencia al exterior', 'Se guardo correctamente el favorito');
            this.onSaveFavorite.emit();
          } else {
            this.dataService.serviceData = res.processBatchId;
            this.onSaveTransfer.emit(this.transferAbroadDetailDto);
          }
        }, error: _err => {
  ...
  handleChangeFrecuentTransfer($event: any) {
    this.smsFavorite = false;
    this.smsFormTA = false;
    if ($event.options === 'IsFrecuent') {
      if ($event.id > 0) {
        ...
        this.transfersAbroadService
          ...
          .subscribe({
            next: (res: TransferAbroadDetailResult) => {
              this.LoadFrecuentResult(res);
              this.smsFavorite = true;
            }
          });
      } else {
        this.isFrecuentTransfer = false;
        this.LoadFrecuentResult(new TransferAbroadDetailResult());
      }
    } else if ($event.options === 'IsFarmoTA') {
      if ($event.id > 0) {
        this.isFrecuentTransfer = true;
        const transferAbroadDto = new GetTransferAbroadDto();
        transferAbroadDto.batch = $event.id;
        this.transfersAbroadService
          .getTransferAbroadDetail(transferAbroadDto)
          .subscribe({
            next: (res: TransferAbroadDetailResult) => {

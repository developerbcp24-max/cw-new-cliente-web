...
import { BallotOfWarrantyAmortizationDto } from '../../../../Services/new-ballot-of-warranty/models/ballot-of-warranty-amortization-dto';
...
import { TimeDepositResult } from '../../../../Services/new-ballot-of-warranty/models/time-deposit-result';
import { RatesDto } from '../../../../Services/new-ballot-of-warranty/models/rates-dto';

@Component({
  selector: 'app-new-other-guarantees',
  standalone: false,
  templateUrl: './new-other-guarantees.component.html',
  styleUrls: ['./new-other-guarantees.component.css'],
  providers: [NewBallotOfWarrantyService,
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => NewOtherGuaranteesComponent),
      multi: true
    }]
})
export class NewOtherGuaranteesComponent implements OnInit {

  @Input() ballot!: BallotOfWarrantyDto;
  @Input() otherWaranties!: ParametersResult;
  @Input() showRoe!: boolean;
  @Input() amount: any;
  ...
  branchOfficesNewBallotOfWarranty = Constants.branchOfficesBallotOfWarranty;
  parametersResult!: ParametersResult;
  typeWarranty = [];
  @ViewChild('warrantyForm') form1!: NgForm;
  @ViewChild('entityNameForm') form2!: NgForm;
  @ViewChild('amortizationForm') form3!: NgForm;
  @ViewChild('dpfForm') form4!: NgForm;
  ...
  constructor(private newBallotOfWarrantyService: NewBallotOfWarrantyService, private parameterService: ParametersService,
  ...
  handleChangeCity() {
    this.ballot.placeOfDelivery = this.agencyObj.name;
    this.ballot.agencyCity = this.agencyObj.name;
    this.ballot.agencyCityCode = this.agencyObj.value;
  }
  ...
  handleChangeWarranty($event: any) {
    ...
    if(this.ballot.typeWarrantyDescription == "PIGNORACION DE FONDOS"){
      this.ballot.typeWarrantyCode = 1
      this.showLine = false;
      this.showPreviosNumber = false;
    }else if(this.ballot.typeWarrantyDescription == "DEPOSITO DE PLAZO FIJO"){
      this.ballot.typeWarrantyCode = 2
    }else if(this.ballot.typeWarrantyDescription == "LINEA DE CREDITO"){
      this.ballot.typeWarrantyCode = 3
      this.showLine = true;
      this.showPreviosNumber = true;
    }
    ...
    this.newBallotOfWarrantyService.getRates(dto)
      .subscribe({next: resp => {
  ...
  handleValidateComision() {
    if (this.ballot.typeWarranty === 'LRC') {
      ...
      this.newBallotOfWarrantyService.getTimeDeposit().subscribe({next: resp => {
  ...
  validateForms() {
    ...
    this.handleCompleteVerify(isValidLRC,isValidAmortization);
  }

  handleCompleteVerify(isValidLRC:boolean,isValidAmortization:boolean){

...
import { RatesResult } from '../../../Services/new-ballot-of-warranty/models/rates-result';
import { ParametersResult } from '../../../Services/new-ballot-of-warranty/models/parameters-result';

@Component({
  selector: 'app-new-ballot-of-warranty',
  standalone: false,
  templateUrl: './new-ballot-of-warranty.component.html',
  styleUrls: ['./new-ballot-of-warranty.component.css'],
  providers: [NewBallotOfWarrantyService, UtilsService, ExchangeRatesService]
})
export class NewBallotOfWarrantyComponent implements OnInit {
  ...
  constructor(private newBallotOfWarrantyService: NewBallotOfWarrantyService, private messageService: GlobalService,
  ...
  ngOnInit() {
    ...
    this.initBallot();
    this.newBallotOfWarrantyService.getParameters().subscribe({next: resp => {
    ...
    .subscribe( {next: response => this.ballot.saleExchangeRate = response.purchase});
  }
  ...
  handleSave() {
    this.newBallotOfWarrantyService.Save(this.ballot).subscribe({next: (_resp) => {
      //not more

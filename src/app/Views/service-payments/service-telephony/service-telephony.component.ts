...
@Component({
  selector: 'app-service-telephony',
  standalone: false,
  templateUrl: './service-telephony.component.html',
  styleUrls: ['./service-telephony.component.css'],
  providers: [ServicePaymentsService, UtilsService]
})
export class ServiceTelephonyComponent implements OnInit {
  ...
  ngOnInit() {
    this.accountRequest = new AccountDto({
      ...
      operationTypeId: [OperationType.pagoDeServiciosTelefonía],
  ...
  handleGroupChanged($event: number) {
    switch ($event) {
      case 0: this.getDebtRequest.service = '3'; break;
      case 1: this.getDebtRequest.service = '4'; break;
    }
  ...
  handleInformation() {
    this.paymentInformation.operationTypeName = this.constants.telephonyService;

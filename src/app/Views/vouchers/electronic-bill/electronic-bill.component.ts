...
export class ElectronicBillComponent implements OnInit {
  ...
  SearchBill() {
    if (this.dateRange.isValid || this.typeFilter === 1) {
      ...
      this.billService.GetListBill(this.requestHeader).subscribe({next: resp => {
  ...
  handleShowDetails($number: string, $code: string) {
    ...
    this.billService.GetDetailBill(this.requestHeader).subscribe({next: resp => {
      this.utilsService.donwloadReport('Factura ' + $number.trim() + '.pdf', resp);
    }, error: _err => this.messageService.info('No se pudo generar el reporte: ', 'Por favor intente mas tarde.')});
  }

  handleDonwloadRarDetail() {
    this.billService.GetBillPerMonth(this.requestHeader).subscribe({next: resp => {
  ...
  handleExportListBill() {
    this.billService.GetListBillReport(this.requestHeader).subscribe({next: resp => {

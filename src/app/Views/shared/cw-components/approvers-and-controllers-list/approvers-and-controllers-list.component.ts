...
@Component({
  selector: 'app-approvers-and-controllers-list',
  standalone: false,
  templateUrl: './approvers-and-controllers-list.component.html',
  styleUrls: ['./approvers-and-controllers-list.component.css'],
  providers: [ApproversAndControllersService, UtilsService]
})
export class ApproversAndControllersListComponent implements OnInit, OnChanges {
    ...
    if (currentUser.authorize_operation) {
      for (const amount of this.amounts) {
        ...
        for (const approverId of this.data.approvers) {
          const approver: any = this.approvers.find(item => item.id === approverId);
